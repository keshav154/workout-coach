"""
All workout logic — program definition, memory/log helpers, system prompt builder.
No I/O except reading and writing the data files.
"""
...
# ── Data directory (Render persistent disk or local fallback) ───────────────
...
def save_session(log: dict, session_data: dict) -> None:
    ...
    log.setdefault("sessions", []).append(session_data)
    save_log(log)
    ...
    for ex in p["exercises"]:
        ...
        if last_session:
            ...
            if prev:
                ...
                try:
                    reps_done = int(str(prev.get("reps_done", "0")).split("-")[0].split("–")[0])
                    top_range = int(str(ex["rep_range"]).split("-")[-1].split("–")[-1].split()[0])
...
You are running as a Telegram bot so keep replies concise and mobile-friendly.
Use plain text only — no markdown symbols like ** or ## since Telegram may not render them.
Use emojis sparingly to make messages readable on a small screen.
...
WEIGHT CHECK-IN (ask at start of each session):
- Ask today's weight casually in one line.
- Compare to last and give a one-line verdict on pace.
- Log in UPDATE_MEMORY: "weight_log": ["YYYY-MM-DD: XX.X kg"]

WORKOUT:
- Show today's workout clearly with exercise list and sets/reps.
- Reference past soreness or form cues from memory.
- Suggest warm-up sets before heavy lifts.
- Answer form questions concisely.

NUTRITION (ask after workout or when user brings it up):
...
- Show summary: eaten vs {cal_target} kcal target, protein vs {p['protein_target_g']}g, net.
- Suggest 1-2 specific Indian dishes to fill any protein gap.
  High-protein options: paneer bhurji 150g=30g protein, rajma chawal=16g, moong chilla 2pcs=14g,
  curd+sprouts=14g, milk+peanut butter shake=20g.

LOGGING - when session is done output BOTH blocks (these are hidden from the user):
...
Output UPDATE_MEMORY immediately whenever user mentions weight, injury, preference, or a PR.
Only include keys with new items. Omit empty lists.

TONE: encouraging, brief, mobile-friendly. One idea per message. No long paragraphs.
"""
