...
- **Micronutrient tracking**: beyond the 4 macros, the Fuel tab tracks fiber, iron, calcium and B12 (the ones that matter most on a vegetarian diet) — derived from your logged foods via the database, shown vs daily targets with low-nutrient flags, and the coach proactively suggests veg-friendly sources when something's running short
- **Verified food database**: common Indian vegetarian foods (roti, dal, paneer, rajma, whey, …) have known per-portion macros, so logging "2 roti and dal" pulls consistent, repeatable numbers instead of a fresh LLM estimate that drifts — the coach checks the DB first (a `lookup_food_macros` tool) and only estimates what isn't in it. Foods you log a lot get cached so they stay consistent too, and known foods cost no LLM tokens
...
- **Periodization / mesocycles**: training runs in structured blocks (a learned number of weeks) — base → accumulation with a planned working-set ramp → a scheduled deload week — shown on the Workout tab as a week-by-week strip, and the coach frames each session within the block. The block length is a learned param the weekly self-tuner can adapt
- **Muscle freshness & smart day pick**: a per-muscle recovery model (0-100, decaying with training load and recovering over muscle-specific days) shows which muscles are still fatigued and recommends the program day that hits the freshest ones — if that differs from the scheduled rotation day, the Workout tab and coach say so (Fitbod's core paid feature). Deterministic, no tokens
...
- **Outcome feedback loop (self-improving)**: the coach grades its OWN autonomous decisions — did an auto-deload actually clear the plateau (next-session e1RM), did a calorie/maintenance change move weight toward your goal — then distils the pattern into a durable observation and feeds it into the weekly self-tuner. This is the step from self-adjusting (react to data) to self-improving (react to your own results); outcomes are shown on the "Coach adaptations" card
...
- **Quick logging without chat**: inline expense form on the Money tab; one-tap frequent-meal chips and "repeat yesterday's meals" on the Fuel tab, plus a searchable "Reuse a Past Meal" library of EVERY food you've ever logged (deduped, most-logged first) so any meal is one tap away without retyping
